...
class LT_RscControlsGroupNoScrollbars : LT_RscControlsGroup {
...
class LT_RscPictureKeepAspect : LT_RscPicture {
	style = 0x30 + 0x800;
};

class LT_RscStructuredText {
	type = 13;
	style = 0;
	x = 0;
	y = 0;
...
class LT_RscButtonTextOnly : LT_RscButton {
	SizeEx = "(			(			(			((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.8)";
...
class LT_RscButtonMenu : LT_RscShortcutButton
{
	idc = -1;
	type = 16;
	style = "0x02 + 0xC0";
...
class LT_RscShortcutButtonMain : LT_RscShortcutButton {
	idc = -1;
	style = 0;
	default = 0;
	w = 0.313726;
...
class LT_RscListBox
{
	...
	class ListScrollBar: LT_ScrollBar
	{
		color[] = {1,1,1,1};
		autoScrollEnabled = 1;
	};
...
class LT_RscBackground : LT_RscText {
	type = 0;
	IDC = -1;
	style = 512;
	shadow = 0;
